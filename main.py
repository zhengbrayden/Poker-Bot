...
Channels = {}
cmds = {'call', 'join', 'create', 'fold', 'info', 'info_lobby', 'start', 'raise', 'leave'} #set of all possible commands
@client.event
async def on_ready():
...
async def on_message(message):
    if (message.author == client.user) or (not message.content.startswith('-')) or message.author.bot:
        return
    ...
    if cmd == 'info_lobby':
        ...
        await info_lobby(client, message, args, Channels)
    elif cmd == 'create':
        ...
        if cmd == 'join':
            ...
            if cmd == 'leave':
                ...
                await info(client, message, player)
